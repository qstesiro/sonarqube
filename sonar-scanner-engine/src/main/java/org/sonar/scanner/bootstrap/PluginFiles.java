...
public class PluginFiles {
        ...
        try (WsResponse response = wsClient.call(request)) {
            ...
            if (!expectedMd5.isPresent()) {
                throw new IllegalStateException(
                    format(
                        "Fail to download plugin [%s]. Request to %s did not return header %s",
                        plugin.key, response.requestUrl(), MD5_HEADER
                    )
                );
            }
            ...
            if (!expectedMd5.get().equals(effectiveTempMd5)) {
                throw new IllegalStateException(
                    format(
                        "Fail to download plugin [%s]. File %s was expected to have checksum %s but had %s",
                        plugin.key, downloadedFile, expectedMd5.get(), effectiveTempMd5
                    )
                );
            }
            ...
            if (compression.isPresent() && PACK200.equals(compression.get())) {
                ...
                cacheMd5 = response.header(UNCOMPRESSED_MD5_HEADER).orElseThrow(
                    () -> new IllegalStateException(
                        format(
                            "Fail to download plugin [%s]. Request to %s did not return header %s.",
                            plugin.key, response.requestUrl(), UNCOMPRESSED_MD5_HEADER
                        )
                    )
                );
            } else {
            ...
            throw new IllegalStateException(
                format(
                    "Fail to download plugin [%s]. Request to %s returned code %d.",
                    plugin.key, e.url(), e.code()
                )
            );
        }
    }
    ...
    private static void downloadBinaryTo(InstalledPlugin plugin, File downloadedFile, WsResponse response) {
        try (InputStream stream = response.contentStream()) {
            ...
            throw new IllegalStateException(
                format("Fail to download plugin [%s] into %s", plugin.key, downloadedFile), e
            );
        }
    }
    ...
    private File unpack200(String pluginKey, File compressedFile) {
        ...
        try (
            InputStream input = new GZIPInputStream(
                new BufferedInputStream(FileUtils.openInputStream(compressedFile))
            );
            JarOutputStream output = new JarOutputStream(
                new BufferedOutputStream(FileUtils.openOutputStream(jar))
            )
        ) {

...
public class ProcessProperties {
    ...
    public enum Property {
        ...
        SEARCH_JAVA_OPTS("sonar.search.javaOpts",
        ...
        // ???
        ELASTIC_HOST("sonar.elastic.host"),
        ELASTIC_PORT("sonar.elastic.port"),
        ELASTIC_USER("sonar.elastic.user"),
        ELASTIC_PASSWORD("sonar.elastic.password"),
        ...
        // ???
        CE_WORKER_COUNT("sonar.worker.count", "0"),
        CE_CORE_MULTIPLE("sonar.core.multiple", "0"),
        ...
        for (CoreExtension ext : extensions) {
            for (Map.Entry<String, String> property : ext.getExtensionProperties().entrySet()) {
                if (propertyDefaults.put(property.getKey(), property.getValue()) != null) {
                    throw new IllegalStateException(format("Configuration error: property definition named '%s'" +
                                                           "found in multiple extensions.",
                                                           property.getKey()));
                }
            }
        }
    ...
    private static void fixPortIfZero(Props props, String addressPropertyKey, String portPropertyKey) {
        ...
        if ("0".equals(port)) {
            ...
            int allocatedPort = NetworkUtilsImpl.INSTANCE.getNextAvailablePort(address)
                .orElseThrow(() -> new IllegalStateException("Cannot resolve address [" +
                                                             address +
                                                             "] set by property [" +
                                                             addressPropertyKey +
                                                             "]"));
    ...
    private static void fixEsTransportPortIfNull(Props props) {
        ...
        if (port == null) {
            int allocatedPort = NetworkUtilsImpl.INSTANCE
                .getNextAvailablePort(InetAddress
                                      .getLoopbackAddress()
                                      .getHostAddress())
                .orElseThrow(() -> new IllegalStateException("Cannot resolve address for Elasticsearch " +
                                                             "TCP transport port"));

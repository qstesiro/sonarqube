...
public class CeProcessingSchedulerImpl implements CeProcessingScheduler {
    ...
    public void startScheduling() {
        for (ChainingCallback chainingCallback : chainingCallbacks) {
            ListenableScheduledFuture<CeWorker.Result> future = executorService
                .schedule(chainingCallback.worker,
                          delayBetweenEnabledTasks,
                          timeUnit);
    ...
    public void gracefulStopScheduling() {
        ...
        try {
            ...
            if (ceWorkerController.hasAtLeastOneProcessingWorker()) {
                LOG.info("Graceful stop period ended but some in-progress" +
                         " task did not finish. Tasks will be interrupted.");
            }
            interruptAllWorkers();

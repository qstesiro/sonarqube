...
public class ProcessLauncherImpl implements ProcessLauncher {
    ...
    public ProcessLauncherImpl(AppSettings settings, File tempDir) {
        this(
            settings,
            tempDir,
            new AllProcessesCommands(tempDir),
            JavaLangProcessBuilder::new
        );
    }
    ...
    ProcessLauncherImpl(
        ...
        File tempDir,
        AllProcessesCommands allProcessesCommands,
    ...
    public ManagedProcess launch(AbstractCommand command) {
        EsInstallation esInstallation = null;
        if (!isExternalElastic()) {
            esInstallation = command.getEsInstallation();
        ...
        try {
            if (processId == ProcessId.ELASTICSEARCH) {
                ...
                EsConnectorImpl esConnector = new EsConnectorImpl(
                    singleton(
                        HostAndPort.fromParts(
                            esInstallation.getHost(),
    ...
    @Override
    public boolean isExternalElastic() {
        Props props = settings.getProps();
        return (!props.value(ELASTIC_HOST.getKey(), "").equals("") &&
                !props.value(ELASTIC_PORT.getKey(), "").equals("") &&
                !props.value(ELASTIC_USER.getKey(), "").equals("") &&
                !props.value(ELASTIC_PASSWORD.getKey(), "").equals(""));
    }
        ...
        try {
            ...
            throw new IllegalStateException(
                format(
                    ...
                    esScriptCommand.getProcessId().getKey()
                ), e
            );
        }
    }
            ...
            .forEach(outdatedDir -> {
                    if (outdatedDir.exists()) {
                        ...
                        try {
                            ...
                            if (outdatedDir.exists()) {
                                LOG.info(
                                    "Failed to delete outdated search index data directory {}",
                                    outdatedDir
                                );
                            }
                        } catch (IOException e) {
    ...
    private static void writeConfFiles(EsInstallation esInstallation) {
        ...
        try (
            FileOutputStream fileOutputStream = new FileOutputStream(
                esInstallation.getLog4j2PropertiesLocation()
            )
        ) {
            esInstallation.getLog4j2Properties().store(
                fileOutputStream,
    ...
    private static <T extends AbstractCommand> void logLaunchedCommand(
    ...
    private ProcessBuilder create(EsScriptCommand esScriptCommand) {
        ...
        requireNonNull(
            esInstallation,
            () -> {
                return "No Elasticsearch installation configuration is available for the command of type " +
                    esScriptCommand.getClass();
            }
        );
    ...
    private <T extends JvmOptions> ProcessBuilder create(JavaCommand<T> javaCommand) {
        ...
        if (javaCommand.getReadsArgumentsFromFile()) {
            ...
            javaCommand.getArguments().forEach(
                (key, value) -> {
    ...
    private File buildPropertiesFile(JavaCommand javaCommand) {
        ...
        try {
            ...
            try (OutputStream out = new FileOutputStream(propertiesFile)) {
                props.store(
                    out, format(
                        "Temporary properties file for command [%s]",

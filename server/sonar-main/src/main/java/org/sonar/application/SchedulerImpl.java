...
public class SchedulerImpl implements
                           Scheduler,
    ...
    private void tryToStartAll() throws InterruptedException {
        if (!processLauncher.isExternalElastic()) {
            tryToStartEs();
        }
        tryToStartWeb();
        tryToStartCe();
    ...
    private void tryToStartProcess (
        ManagedProcessHandler processHandler,
        Supplier<AbstractCommand> commandSupplier
        ...
        try {
            ...
            LOG.trace(
                "Failed to start process [{}] (currentThread={})",
    ...
    private static void logThreadRecreated(String threadType, Thread existingThread) {
        if (LOG.isDebugEnabled()) {
            ...
            LOG.debug(
                "{} thread was not null (currentThread={},existingThread={})",

...
public class ClusterSettings implements Consumer<Props> {
    ...
    private void checkClusterProperties(Props props) {
        ...
        case SEARCH:
            ...
            AddressAndPort searchHost = parseAndCheckHost(CLUSTER_NODE_SEARCH_HOST,
                                                          requireValue(props, CLUSTER_NODE_SEARCH_HOST));
            ensureLocalButNotLoopbackAddress(CLUSTER_NODE_SEARCH_HOST, searchHost);
            AddressAndPort esHost = parseAndCheckHost(CLUSTER_NODE_ES_HOST,
                                                      requireValue(props, CLUSTER_NODE_ES_HOST));
    ...
    private void checkClusterNodeHost(Props props) {
        AddressAndPort clusterNodeHost = parseAndCheckHost(CLUSTER_NODE_HOST,
    ...
    private void checkClusterSearchHosts(Props props) {
        Set<AddressAndPort> searchHosts = parseHosts(CLUSTER_SEARCH_HOSTS,
                                                     requireValue(props, CLUSTER_SEARCH_HOSTS));
    ...
    public static NodeType toNodeType(Props props) {
        ...
        if (!NodeType.isValid(nodeTypeValue)) {
            throw new MessageException(format("Invalid value for property %s: [%s], only [%s] are allowed",
                                              CLUSTER_NODE_TYPE.getKey(), nodeTypeValue,
                                              Arrays.stream(NodeType.values())
                                              .map(NodeType::getValue).collect(joining(", "))));
        }
    ...
    private static void ensureNoSearchNodeForbiddenSettings(Props props) {
        ...
        if (!violations.isEmpty()) {
            throw new MessageException(format("Properties [%s] are not allowed when running " +
                                              "SonarQube in cluster mode.",
    ...
    private static void ensureNotH2(Props props) {
        ...
        if (trimmedJdbcUrl == null ||
            trimmedJdbcUrl.isEmpty() ||
            trimmedJdbcUrl.startsWith("jdbc:h2:")) {
    ...
    private void ensureNotLoopbackAddresses(Property property, Set<AddressAndPort> hostAndPorts) {
        ...
        if (!loopbackAddresses.isEmpty()) {
            throw new MessageException(format("Property %s must not contain a loopback address: %s",
                                              property.getKey(),
                                              loopbackAddresses.stream()
                                              .map(AddressAndPort::getHost)
                                              .sorted()
    ...
    private void ensureLocalButNotLoopbackAddress(Property property, AddressAndPort addressAndPort) {
        ...
        if (!network.isLocal(host) || network.isLoopback(host)) {
            throw new MessageException(format("Property %s must be a local non-loopback address: %s",
                                              property.getKey(), addressAndPort.getHost()));
        }
    }
    ...
    private static void ensureEitherPortsAreProvidedOrOnlyHosts(Property property, Set<AddressAndPort> addressAndPorts) {
        Set<AddressAndPort> hostsWithoutPort = addressAndPorts.stream()
            .filter(t -> !t.hasPort())
            .collect(toSet());
        if (!hostsWithoutPort.isEmpty() &&
            hostsWithoutPort.size() != addressAndPorts.size()) {
            throw new MessageException(format("Entries in property %s must not mix 'host:port' and 'host'. " +
                                              "Provide hosts without port only or hosts with port only.",
                                              property.getKey()));
        }
    }
    ...
    public static boolean shouldStartHazelcast(AppSettings appSettings) {
        return isClusterEnabled(appSettings.getProps()) &&
            toNodeType(appSettings.getProps()).equals(NodeType.APPLICATION);
    }
